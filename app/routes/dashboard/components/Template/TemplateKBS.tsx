      ...
      {content.meta.labelSort.length > 0 ? (
        <div className="w-full px-3">
          {content.meta.labelSort.map((label) => {
            switch (label) {
              case "basic":
                return content.basic.filter(
                  (item) => item.key !== "name" && item.key !== "job",
                ).length > 0 ? (
                  <div key={label}>basic</div>
                ) : (
                  <></>
                );
              case "education":
                return content.education.length > 0 ? (
                  <div key={label}>education</div>
                ) : (
                  <></>
                );
              case "job":
                return content.job.length > 0 ? (
                  <div key={label}>job</div>
                ) : (
                  <></>
                );
              case "project":
                return content.project.length > 0 ? (
                  <div key={label}>project</div>
                ) : (
                  <></>
                );
              case "skill":
                return content.skill ? <div key={label}>skill</div> : <></>;
              case "custom":
                return content.custom.label && content.custom.value ? (
                  <div key={label}>custom</div>
                ) : (
                  <></>
                );
              default:
